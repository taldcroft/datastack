...
for phafile in phafiles:
    ds.load_pha([], os.path.join(datadir, phafile))
...
vals_iter = izip(count(1),
                 ...
                 bkg_models,
                 ds.get_bkg_scale([]))
for id_, src_model, rsp, bkg_model, bkg_scale in vals_iter:
    ds.set_full_model(id_,  rsp(src_model) + bkg_scale * bkg_model)
    ds.set_bkg_full_model(id_, bkg_model)

## Fit background
ds.notice(0.5, 8.)
ds.set_method("neldermead")
ds.set_stat("cash")
